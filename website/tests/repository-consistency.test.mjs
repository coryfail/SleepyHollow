...
const legacyRequirementPath = (path) => {
  ...
  if (path === "requirements/application.req.md") {
    return "requirements/application.md";
  }
  return path.replace(/[^/]+\.req\.md$/, "requirements.md");
};

const historicalRequirement = (path) => {
  for (const revision of ["HEAD", "HEAD^"]) {
    try {
      return execFileSync("git", [
        "show",
        `${revision}:${legacyRequirementPath(path)}`,
      ], {
        cwd: repository,
        encoding: "utf8",
      ...
      // A shallow CI checkout or a later post-migration revision has no legacy
      // source artifact to compare. Current governance checks still apply.
    }
  }
  return undefined;
};

const normalizeNamedRequirementText = (text) =>
  text
    .replaceAll(
...
const governedContent = (document, path) => {
  ...
  assert.ok(
    governedFrontmatter,
    `${path} must expose YAML frontmatter before governance`,
  );
  const normalizedFrontmatter = governedFrontmatter[0].replace(
    /^status:[^\n]*\n/m,
    "",
  );
  const normalized = normalizedFrontmatter +
    governed.slice(governedFrontmatter[0].length);
  assert.notEqual(
    normalized,
    governed,
    `${path} must expose one top-level status projection`,
  );
...
test("AC-REPO-001 AC-REPO-002 · requirements use one digest and governance boundary", () => {
  ...
  assert.ok(
    paths.length > 20,
    "the complete requirement set must be discoverable",
  );
  let endpoints = 0;
  for (const path of paths) {
    ...
    assert.ok(
      ["draft", "approved", "verified"].includes(metadata.status),
      `${path} has invalid status`,
    );
    assert.doesNotMatch(
      record.slice("## Governance record\n".length),
      /^## /m,
      `${path} has content after governance`,
    );
    const headings = endpoint ? ["Approval"] : [
      "Approval",
      "Criterion mapping",
      "Red-state evidence",
      "Verification",
    ...
    assert.match(
      record,
      new RegExp(
        `Governed-content digest:[\\s\\S]{0,80}sha256:${sha256(normalized)}`,
      ),
  ...
  assert.ok(
    endpoints > 0,
    "endpoint requirements must be covered by this sweep",
  );

  const migrationRequirement = read("named-requirement-files.req.md");
  const { normalized, record } = governedContent(
    migrationRequirement,
    "named-requirement-files.req.md",
  );
  assert.match(
    record,
    new RegExp(
  ...
  assert.match(
    canonicalRules,
    /omit[\s\S]{0,160}top-level[\s\S]{0,80}`?status/is,
  );
  assert.match(
    canonicalRules,
    /status[\s\S]{0,200}(?:projection|routing)[\s\S]{0,200}digest/is,
  );
});

test("AC-REPO-003 · requirement, criterion, dependency, and decision identities resolve", () => {
  const documents = requirementPaths().map((path) => ({
    path,
    document: read(path),
  }));
  ...
  for (const item of documents) {
    ...
    assert.equal(
      identities.has(metadata.id),
      false,
      `duplicate requirement ID ${metadata.id}`,
    );
  ...
  for (const item of documents) {
    ...
    for (const dependency of metadata.depends_on) {
      assert.ok(
        identities.has(dependency),
        `${item.path} has unresolved dependency ${dependency}`,
      );
    }
    for (const decision of metadata.open_decisions ?? []) {
      assert.match(
        read("requirements/application.req.md"),
        new RegExp(`\\| ${decision} \\|`),
        `${item.path} has unresolved ${decision}`,
      );
    }
  }
  ...
  for (const item of documents) {
    for (const match of item.document.matchAll(/^- (AC-[A-Z0-9-]+-\d{3}):/gm)) {
      assert.equal(
        criteria.has(match[1]),
        false,
        `duplicate criterion ID ${match[1]}`,
      );
  ...
  assert.doesNotMatch(
    read("website/src/pages/sgad/sgad.req.md"),
    /^- AC-SGAD-/m,
  );
});
...
test("AC-REPO-004 · requirement placement follows behavioral ownership", () => {
  ...
  assert.deepEqual(
    walk(applicationDirectory).map((path) =>
      relative(applicationDirectory, path)
    ).sort(),
    ["application.req.md"],
  );
  ...
  assert.match(
    guidance,
    /requirements\/application\.req\.md[\s\S]{0,200}(?:product|application)-wide|(?:product|application)-wide[\s\S]{0,200}requirements\/application\.req\.md/i,
  );
  assert.match(
    guidance,
    /component[\s\S]{0,200}\.req\.md[\s\S]{0,200}(?:colocat|own|beside)/i,
  );
  assert.match(
    guidance,
    /repository-wide behavior[\s\S]{0,200}(?:root-level|\.req\.md)/i,
  );
});
...
test("AC-REPO-005 · current naming and status claims are honest", () => {
  ...
  assert.doesNotMatch(
    ...
    /main contains production-ready code/i,
  );
  ...
  assert.doesNotMatch(
    currentGuidance,
    /verification-report\.md|verification reports? template/i,
  );
...
test("AC-REPO-006 · docs, skill, templates, links, and metadata stay aligned", () => {
  for (
    const name of ["application-requirements.md", "component-requirements.md"]
  ) {
    assert.equal(
...
test("AC-REPO-007 · website canonical values and assets are local and centralized", () => {
  const documents = [
    ...
    read("website/sgad/index.html"),
  ].join("\n");
  assert.doesNotMatch(documents, /fonts\.googleapis\.com|fonts\.gstatic\.com/);
  assert.match(
    read("website/src/site.ts"),
    /github\.com\/coryfail\/SleepyHollow/,
  );
  ...
  for (
    ...
    assert.match(
      read(path),
      /from ["'](?:\.\.\/)*site["']/,
      `${path} must use shared site values`,
    );
    assert.doesNotMatch(
      read(path),
      /const repositoryUrl|github\.com\/coryfail\/SleepyHollow/,
    );
  }
});
...
test("AC-REPO-010 · product activation remains governed after cleanup", () => {
  const componentPaths = requirementPaths().filter((path) =>
    /^(?:cli|core|skills\/sleepy-hollow)\//.test(path)
  );
  for (const path of componentPaths) {
    ...
    if (currentMetadata.status === "draft") {
      const prior = historicalRequirement(path);
      ...
      assert.deepEqual(
        currentMetadata.depends_on,
        previousMetadata.depends_on,
        `${path} draft dependencies changed`,
      );
      assert.deepEqual(
        currentMetadata.open_decisions ?? [],
        previousMetadata.open_decisions ?? [],
        `${path} draft decisions changed`,
      );
      continue;
    }
...
test("AC-NRF-002 AC-NRF-009 · every current governed artifact has a named requirement path", () => {
  ...
  const legacy = files.filter((path) =>
    path === "requirements.md" || path.endsWith("/requirements.md")
  );
...
test("AC-NRF-012 · framework release surfaces report 0.3.6", () => {
  assert.equal(JSON.parse(read("package.json")).version, "0.3.6");
  assert.equal(JSON.parse(read("website/package.json")).version, "0.2.0");
  assert.match(read("cli/dispatcher.ts"), /CLI_VERSION = "0\.3\.6"/);
  assert.match(read("cli/create/create.ts"), /FRAMEWORK_VERSION = "0\.3\.6"/);
  assert.match(
...
test("AC-NRF-013 · contribution workflow uses main without a development branch", () => {
  ...
  assert.match(
    contributing,
    /feature branches from an up-to-date `main` branch/i,
  );
  assert.match(contributing, /pull request into `main`/i);
  assert.doesNotMatch(
    contributing,
    /`development`|origin\/development|git switch development/,
  );
});
...
test("AC-REPO-012 · generated and ignored output does not enter the repository", () => {
  for (const path of repositoryFiles()) {
    assert.equal(
      statSync(resolve(repository, path)).isFile(),
      true,
      `${path} is not a file`,
    );
