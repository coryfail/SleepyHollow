...
test("AC-NRF-012 · framework release surfaces report 0.3.1", () => {
  assert.equal(JSON.parse(read("package.json")).version, "0.3.1");
  assert.equal(JSON.parse(read("website/package.json")).version, "0.2.0");
  assert.match(read("cli/dispatcher.ts"), /CLI_VERSION = "0\.3\.1"/);
  assert.match(read("cli/create/create.ts"), /FRAMEWORK_VERSION = "0\.3\.1"/);
  assert.match(
